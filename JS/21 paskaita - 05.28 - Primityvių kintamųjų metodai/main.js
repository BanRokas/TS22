...
console.groupCollapsed('rimtesnis pvz');
...
let capitalizeFirstLetter = (text) => {
  ...
  for(let i = 0; i < textArray.length; i++){
    // console.log(textArray[i].charAt(0).toUpperCase() + textArray[i].slice(1));
    returnString+=textArray[i].charAt(0).toUpperCase() + textArray[i].slice(1)+' ';
  }
  return returnString.trim();
  // return text.split(' ').map(zodis => zodis[0].toUpperCase() + zodis.slice(1)).join(' ');
}
